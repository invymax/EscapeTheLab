...
public class InventoryUI : MonoBehaviour
{
    ...
    void OnSlotDoubleClick(int idx)
    {
        ...
        {
            Debug.Log("Нельзя использовать: условие не выполнено (например, полное HP или эффект уже активен).");
            return;
        }
    ...
    bool CanUseItem(ItemDefinition item)
    {
        ...
        {
            case ItemType.Heal:
                // Нельзя если здоровье уже полное
                if (playerHealth != null && playerHealth.IsFull) return false;
                return true;

            case ItemType.InvisibilityBracelet:
                // Нельзя повторно, пока эффект активен
                if (playerEffects != null && playerEffects.IsInvisible) return false;
                return true;
    ...
    void OnUsePressed()
    {
        ...
        if (!CanUseItem(slot.item))
        {
            Debug.Log("Use: предмет нельзя применить (например, полное HP или невидимость уже активна).");
            UpdateDetailUseState();
            return;
        }
        ...
        {
            // не применился (например, проверка внутри PlayerEffects)
            UpdateDetailUseState();
        }
    }
